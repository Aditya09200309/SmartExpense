export const analyticsEngine = {
  computeTrustScore(avgTimeToSettleHours: number, totalDelayedPayments: number): number {
    let score = 100
  ...
  computeGroupHealth(staleDebtVolume: number, totalExpensesVolume: number, fairnessIndex: number): number {
    let health = 100
  ...
  computeFairnessIndex(userPayments: number[], totalExpenses: number): number {
    if (totalExpenses === 0 || userPayments.length === 0) return 0
  ...
  computePersonalityTrait(avgTimeToSettleHours: number, totalDelayedPayments: number, fairnessIndex: number, sampleSize: number): { trait: string, confidence: number } | null {
    if (sampleSize < 3) return null // Not enough data to be confident
